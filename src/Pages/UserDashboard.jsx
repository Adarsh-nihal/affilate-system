...
const UserDashboard = () => {
  ...
  const fetchAffiliates = async () => {
    try {
      setIsLoading(true);
      const response = await fetchGetRequest(`/api/affiliate/get-affiliate-dashboard-data?referral_code=${affiliateCode}`
      );
  ...
  <h3 className="text-lg font-semibold mt-10 text-[#F9BA1F]">All Users</h3>
  <div className="space-y-2 mt-2 min-h-[250px]">
    {affiliates?.users?.map((user) => (
      <div key={user._id} className="flex justify-between items-center bg-[#32383D] p-2 md:p-4 rounded-lg">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h4 className={`font-semibold text-sm md:text-lg ${user.is_blocked ? 'text-red-500' : 'text-white'}`}>
              {user.username}
            </h4>
        ...
        <div className="flex flex-col items-end">
          <p className="font-semibold text-sm md:text-lg text-white">Amount: {user.amount} {user.currency}</p>
          <p className="text-gray-400 text-xs md:text-sm">Country: {user.country}</p>
        </div>
      </div>
    ))}
  </div>
